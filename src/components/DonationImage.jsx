...
const DonationImageVerifier = ({ onVerified, onReset, itemName = 'item' }) => {
  ...
  const analyzeImage = useCallback(async (file) => {
    ...
    try {
      ...
      const prompt = `You are a donation quality inspector for a charity platform called KindNest.

Analyze this image of a donated ${itemName} and assess its physical condition.

Respond ONLY with a valid JSON object — no markdown, no extra text — in this exact format:
{
  "label": "very_good" | "good" | "poor",
  "confidence": <number between 0.0 and 1.0>,
  "summary": "<one concise sentence describing the condition>",
  "reasons": ["<reason 1>", "<reason 2>"]
}

Labeling guide:
- "very_good": item looks clean, undamaged, minimal wear, ready to use
- "good": item is usable but shows some wear, minor stains or small imperfections
- "poor": item is visibly damaged, heavily stained, torn, broken, or otherwise unsuitable for donation
